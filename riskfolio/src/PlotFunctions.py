...
def plot_frontier(
    ...
    #Dario
    if rm in ["CVaR"]:
        x_label = rm_names[item] + " (" + rm + " 1M)"
    elif rm in ["TG", "EVaR", "RLVaR", "CVRG", "TGRG", "CDaR", "EDaR", "RLDaR"]:
        x_label = (
    ...
    for i in range(w_frontier.shape[1]):
        try:
            ...
            if rm in ["CVaR"]:
                risk = risk * 20 ** 0.5
            elif rm not in ["MDD", "ADD", "CDaR", "EDaR", "RLDaR", "UCI"]:
                risk = risk * t_factor ** 0.5
...
def plot_hist(
    ...
    risk = [
        ...
        -rk.TG(a, alpha, a_sim),
        #Dario -rk.EVaR_Hist(a, alpha)[0],
        #Dario -rk.RLVaR_Hist(a, alpha, kappa, solver),
        -rk.WR(a),
    ]
    label = [
        ...
        # + "): "
        # + "{0:.2%}".format(risk[8]),
        "Worst Realization: " + "{0:.2%}".format(risk[7]), #Dario 
    ]
    color = [
        "b",
        "r",
...
def plot_drawdown(
    ...
    risk = [
        ...
        -rk.DaR_Abs(a, alpha),
        -rk.CDaR_Abs(a, alpha),
        #Dario -rk.EDaR_Abs(a, alpha)[0],
        #Dario-rk.RLDaR_Abs(a, alpha, kappa, solver),
        -rk.MDD_Abs(a),
    ]
    label = [
        ...
        # + "): "
        # + "{0:.2%}".format(risk[5]),
        "Maximum Drawdown: " + "{0:.2%}".format(risk[4]), #Dario
    ]
...
def plot_table(
    ...
    rowLabels = [
        ...
        "Value at Risk (VaR) (4)", #Dario
        "Conditional Value at Risk (CVaR) (4)", #Dario
        #Dario "Entropic Value at Risk (EVaR) (2)",
        "Tail Gini of Losses (TG) (2)",
        #Dario "Relativistic Value at Risk (RLVaR ) (2)",
        "Worst Realization (2)",
        "Skewness",
        "Kurtosis",
        "",
    ...
    indicators = [
        ...
        rk.VaR_Hist(X, alpha=alpha) * 20**0.5,
        rk.CVaR_Hist(X, alpha=alpha) * 20**0.5,
        rk.TG(X, alpha=alpha, a_sim=a_sim) * t_factor**0.5,
        #Dario rk.EVaR_Hist(X, alpha=alpha)[0] * t_factor**0.5,
        #Dario rk.RLVaR_Hist(X, alpha=alpha, kappa=kappa, solver=solver) * t_factor**0.5,
        ...
        #Dario rk.EDaR_Abs(X, alpha=alpha)[0],
        #Dario rk.RLDaR_Abs(X, alpha=alpha, kappa=kappa, solver=solver),
        rk.MDD_Abs(X),
        "",
        "",
        "",
    ...
    for i in range(len(indicators)):
        if indicators[i] != "":
            if rowLabels[i] in ["Skewness", "Kurtosis"]:
                indicators[i] = "{:.2f}".format(indicators[i]) #Dario
            else:
                indicators[i] = "{:.2%}".format(indicators[i]) #Dario
        if ratios[i] != "":
            ratios[i] = "{:.2f}".format(ratios[i]) #Dario
    ...
    #Dario colWidths = [0.45, 0.275, 0.275]
    ...
    for i in range(0, ncols):
        for j in range(nrows - 3, nrows + 1):   #Dario
